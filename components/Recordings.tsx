...
const VideoCard = ({ video, user, onTriggerLogin }: { video: Video, user: User | null, onTriggerLogin: () => void }) => {
  ...
  useEffect(() => {
    const fetchYouTubeTitle = async () => {
      ...
      try {
        const response = await fetch(`https://noembed.com/embed?url=${video.videoUrl}`);
        const data = await response.json();
        if (data.title) {
          setDisplayTitle(data.title);
        }
      } catch (error) {
        console.warn(`No se pudo obtener el título para el video ${video.id}`, error);
      }
    };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow overflow-hidden border border-slate-100 flex flex-col relative h-full">
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        <h3 className="text-lg font-bold text-slate-900 mb-4 leading-tight">{displayTitle}</h3>
